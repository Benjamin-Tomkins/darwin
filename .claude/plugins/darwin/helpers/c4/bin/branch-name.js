 ...
 */
export function branchName(slugChain, asset) {
    if (slugChain.length === 0)
        throw new Error('slugChain must not be empty');
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
    ...
    let slugChain;
    try {
        slugChain = JSON.parse(rawArg);
        if (!Array.isArray(slugChain) || slugChain.some(s => typeof s !== 'string')) {
            throw new Error('not a string array');
        }
    }
    catch (err) {
        process.stderr.write(`branch-name: invalid slug chain: ${err.message}\n`);
        process.exit(1);
    }
