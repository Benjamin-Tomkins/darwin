...
| R12.8 | When a DSL element carries both `tests:` and `impl:` asset properties, the controller treats them as a **co-evolving pair**: `tests` and `impl` Ralph loops run concurrently; `impl` cannot reach `●` until the tests gate passes (R12.9). Both tasks have independent pairings, circuit breakers, and ⊗/↺ histories. | Must |
| R12.9 | **Tests gate** (evaluated by the controller before running `impl`'s eval pipeline): (a) the `tests` branch must be at `●`; AND (b) the `●` commit on `tests` must post-date the most recent `⊗` commit on `impl`. Both conditions checked via Git commit timestamps on the respective branches. Gate failing → `failure_class: deps-missing`, `consumes_attempt: false` on the `impl` task; no eval pipeline runs. | Must |
| R12.10 | **Gate staleness trigger**: when `impl` produces a new `⊗` after `tests` has already reached `●`, the controller marks the gate stale and triggers a **tests re-evaluation** — re-running the `tests` eval pipeline with the impl task's current `⊗` trailer corpus injected into any rubric judge's context (see R12.11). The `tests` branch does not revert to `◌`; the re-evaluation is a new eval attempt on the existing `tests` branch. | Must |
| R12.11 | **Cross-task experience injection**: on a tests re-evaluation triggered by gate staleness (R12.10), the controller appends a structured block to every rubric eval's judge prompt: the impl task's `⊗` trailer history (attempt number, `problem`, `hypothesis`, `evidence` fields), presented as "implementation failure patterns — assess whether the test suite covers these scenarios." This applies regardless of eval type (command, rubric, or composite); rubric evals use it in the judge prompt; command evals run unchanged but the gate staleness is still checked. | Must |
| R12.12 | Tests re-evaluation attempts triggered by R12.10 are classified `cross-task-reeval` in the attempt log and do **not** count against the `tests` task's `max_attempts_total` circuit breaker. They count against `max_cost_usd` and `max_wall_clock_min`. A separate `max_reeval_attempts` limit (default: 3) prevents runaway re-evaluation loops; exhausting it generates HANDOFF on the `tests` task. | Must |
| R12.13 | The gate check (R12.9) is computed entirely from Git branch history; no external state is required. Controller reads the timestamp of the latest `●` commit on `agent/<slug>/tests` and the latest commit with `Try-Status: fail` on `agent/<slug>/impl` using `git log --format=%ct`. | Must |
| R12.14 | The **tests task agent** uses the standard escalation ladder: entry tier on first attempt, escalating on repeated failure. For cross-task re-evaluation attempts (R12.10), the tests agent starts at `tier-above` entry rather than entry — reflecting that the initial-tier agent did not produce a test suite sufficient to track impl failures. | Must |
| R12.15 | The cross-task rubric eval (R12.11) MUST use `judge_model: top` — the highest available tier — regardless of the tests agent's current tier. Standard evals in the tests pipeline use their declared `judge_model`; only the cross-task rubric is unconditionally top-tier. Rationale: assessing whether a test suite covers an evolving impl failure corpus across subjective and structural dimensions requires broader reasoning than a per-attempt tier-relative judge can provide. | Must |
| R12.16 | The top-tier cross-task judge's `evidence` and `problem` fields are appended to the experience brief of **both** the tests task and the impl task for their next attempts. Tests gets: gaps the judge identified in test coverage. Impl gets: structural or architectural issues surfaced by the judge that were not previously in the impl's failure corpus. This propagates high-tier insight into both Ralph loops without requiring either agent to re-derive it. | Must |
...
| R14.3 | Every workflow-spawning DSL element MUST declare an immutable `"identifier"` property (kebab-case, unique within the parent's children); controller validates uniqueness at parse time and refuses to scaffold on collision | Must |
...
| R14.5 | Branch convention is slash-nested: `agent/<project-identifier>/<element-path>/<asset-key|_index>`. `<element-path>` is the dot-to-slash translation of the chain of `"identifier"` properties walking from the element to the project root | Must |
...
| R14.7 | Exactly one canonical index branch per project: `agent/<project-identifier>/_index`. All `index.adoc` evolution serialises on this branch; concurrent index editing is forbidden. Sub-branches edit asset `.adoc` files only | Must |
