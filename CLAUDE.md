...
- **Folder is flat; hierarchy is in Git branches.** Branches are slash-nested mirroring the chain of DSL `"identifier"` properties walking up from the element to the project root.
- **Every workflow-spawning DSL element MUST declare an immutable `"identifier"` property** (R14.3). Identifier uniqueness is validated within parent's children at parse time. Branch paths and filenames derive from identifiers, not from DSL variable names or display names — this means renaming the display name or DSL variable name doesn't break branches.
...
- **Single canonical index branch** (R14.7). All `index.adoc` evolution serialises on `agent/<project-identifier>/_index`. Concurrent index editing is forbidden — sub-branches edit asset `.adoc` files only, never the index itself.
...
- **Property keys come in two categories.** Asset-reference keys (`detail`, `bdd`, `tests`, `impl`) point at sibling `.adoc` filenames and spawn workflows. Metadata keys (`identifier`, `skills`) carry inline values without spawning a workflow. Don't conflate them.
