...
export function extractStructurizrBlock(adoc: string): string {
  ...
  if (!m) throw new Error('No [source,structurizr] block found in AsciiDoc file');
  return m[2];
}

// ── Tokeniser ──────────────────────────────────────────────────────────────

type TokType = 'WORD' | 'STRING' | 'LBRACE' | 'RBRACE' | 'NL' | 'EOF';
interface Tok { type: TokType; value: string; line: number }
...
function tokenise(src: string): Tok[] {
  ...
  while (i < src.length) {
    ...
    if (ch === ' ' || ch === '\t') { i++; continue; }
    if (ch === '{') { toks.push({ type: 'LBRACE', value: '{', line }); i++; continue; }
    if (ch === '}') { toks.push({ type: 'RBRACE', value: '}', line }); i++; continue; }
    if (ch === '"') {
      let j = i + 1;
    ...
    if (j > i) { toks.push({ type: 'WORD', value: src.slice(i, j), line }); i = j; }
    else i++;
  }
...
class Parser {
  ...
  expect(type: TokType, value?: string): Tok {
    ...
    if (t.type !== type || (value !== undefined && t.value !== value)) {
      throw new Error(
        `Expected ${type}${value ? `(${value})` : ''} at line ${t.line}, got ${t.type}(${JSON.stringify(t.value)})`
      );
    }
    return t;
  }
  ...
  parseElement(): DarwinElement {
    ...
    let description = '';
    if (this.peek().type === 'STRING') description = this.next().value;
...
export function parseIndex(filePath: string): ElementTree {
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  try {
    ...
    process.stderr.write(`parse-index: ${(err as Error).message}\n`);
    process.exit(1);
  }
}
